...
export function logErrorMessage(
  ...
  if (matDialog !== null) {
    ...
    if (
      ...
      serverResponse.error.errors !== undefined
    )
      errorMessage = Object.values(serverResponse.error.errors)[0];
    else errorMessage = context.errorMessage;

    dialogConfig.data = {
      errorMessage: errorMessage,
    };

    let dialogRef = matDialog.open(ErrorHandlerComponent, dialogConfig);
  }
}
