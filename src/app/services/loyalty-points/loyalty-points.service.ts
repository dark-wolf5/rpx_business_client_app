...
const LOYALTY_POINTS_API = rpxGlobals.API + "loyalty-points";
const LOYALTY_POINTS_TIER_API = rpxGlobals.API + "lp-tiers";
const REDEEMABLE_API = rpxGlobals.API + "redeemable";
const USER_API = rpxGlobals.API + "user";
const REWARDS_API = rpxGlobals.API + "reward";
...
export class LoyaltyPointsService {
  ...
  savePercentage(percent: number): Observable<any> {
    const apiUrl = `${LOYALTY_POINTS_API}/set-lp-rate`;

    return this.http
      .post<any>(apiUrl, { percent })
      .pipe(catchError(handleError("set-lp-rate")));
  }
}
