...
export class RewardMenuComponent implements OnInit {
  ...
  async ngOnInit() {
    let userType = await Preferences.get({ key: "rpx_userType" });
    this.userType$.next(parseInt(userType.value, 10));

    this.isLoggedIn$.next(
      (await Preferences.get({ key: "rpx_loggedIn" })).value,
    );
  ...
  private async fetchRewardsCb(resp) {
    ...
    if (
      this.userType$.getValue() === this.eAllowedAccountTypes.Personal ||
      this.isLoggedIn$.getValue() !== "1"
    ) {
      this.userPointToDollarRatio$.next(
        resp.loyalty_point_dollar_percent_value,
      );
