...
import { RewardCreatorService } from "../../../services/rpx-logged-in/business-menu/reward-creator/reward-creator.service";
...
const REWARD_MEDIA_UPLOAD_API_URL = `${rpxGlobals.API}reward/upload-media`;
...
export class RewardCreatorComponent implements OnInit {
  ...
  rewardUploadImage$ = new BehaviorSubject<string>(
  ...
  get isGlobal() {
    return this.rewardCreatorForm.get("is_global").value;
  }
  get tier() {
  ...
  private setRewardLink() {
    this.rewardClaimUrl$.next(
      `${this.qrCodeClaimReward}?&r=${this.reward$.getValue().uuid}&t=claim_reward`,
    );
  }
  ...
  calculatePointValue() {
    ...
    if (pointPercentage === 0 || pointPercentage == null) {
      ...
      this.businessPointsDollarValue$.next(
        ((itemPrice / pointPercentage) * 100).toFixed(2),
      );
    }
  ...
  async startRewardMediaUploader(): Promise<void> {
    Camera.checkPermissions().then((status) => {
      if (status.photos === "granted") {
        ...
        Camera.requestPermissions({ permissions: ["photos"] }).then(
          (status) => {
            if (status.photos === "granted") {
              this.rewardMediaUploaders();
  ...
  async uploadMedia(file: Photo): Promise<void> {
    ...
    const token = await Preferences.get({ key: "rpxcom_session" });
    ...
    this.http
      ...
      .subscribe((event) => {
        if (event.type === HttpEventType.UploadProgress) {
          this.rewardMediaUploadProgress$.next(
  ...
  private rewardMediaUploadFinished(httpResponse: any): void {
    if (httpResponse.success) {
      ...
      this.rewardCreatorForm
        .get("rewardImage")
        .setValue(this.rewardUploadImage$.getValue());
    } else {
  ...
  deleteMe() {
    this.rewardCreatorService
      .deleteMe(this.reward$.getValue())
      .subscribe((resp) => {
