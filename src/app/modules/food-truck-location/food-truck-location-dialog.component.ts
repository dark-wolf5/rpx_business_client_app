...
export class FoodTruckLocationDialogComponent implements OnInit, AfterViewInit {
  ...
  originPhoto$ = new BehaviorSubject<string>(
  ...
  user$: Observable<User> = this.userService.user$.pipe(
    ...
    tap(user => {
      this.originTitle$.next(user.business.name);
    }),
    takeUntilDestroyed()
  );
  ...
  saveLocation() {
    ...
    this.userService.saveLocation(businessInfo).subscribe((resp) => {
      if (resp.success) {
        ...
        this.locationSavedSuccessfullyMessage$.next(
      ...
      (results: GeocoderResult, status) => {
        if (status === "OK") {
          if (results[0]) {
            ...
            this.postalCode$.next(
              this.getAddressComponent(results[0], "postal_code"),
            );
