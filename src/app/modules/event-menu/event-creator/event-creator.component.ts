...
import { EventCreatorService } from "../../../services/rpx-logged-in/event-menu/event-creator/event-creator.service";

const event_MEDIA_UPLOAD_API_URL = `${rpxGlobals.API}event/upload-media`;
const event_MEDIA_MAX_UPLOAD_SIZE = 25e6;
...
export class EventCreatorComponent implements OnInit {
  ...
  get eventType() {
    return this.eventCreatorForm.get("eventType").value;
  }
  get eventValue() {
    return this.eventCreatorForm.get("eventValue").value;
  }
  get eventName() {
    return this.eventCreatorForm.get("eventName").value;
  }
  get eventDescription() {
    return this.eventCreatorForm.get("eventDescription").value;
  }
  get eventImage() {
    return this.eventCreatorForm.get("eventImage").value;
  }

  get f() {
    return this.eventCreatorForm.controls;
  }
  ...
  initeventForm() {
    ...
    const eventDescriptionValidators = [
      Validators.required,
  ...
  saveEvent() {
    ...
    if (this.event === null || this.event === undefined) {
      this.eventCreatorService.saveItem(itemObj).subscribe((resp) => {
        this.saveeventCb(resp);
      });
    } else {
      itemObj.id = this.event.id;

      this.eventCreatorService.updateItem(itemObj).subscribe((resp) => {
        this.saveeventCb(resp);
      });
    }
  }
  ...
  async uploadMedia(files) {
    ...
    let token = await Preferences.get({ key: "rpxcom_session" });

    this.http
      .post(event_MEDIA_UPLOAD_API_URL, formData, {
        reportProgress: true,
        observe: "events",
      ...
      .subscribe((event) => {
        if (event.type === HttpEventType.UploadProgress) {
          this.eventMediaUploadProgress = Math.round(
  ...
  deleteMe() {
    this.eventCreatorService.deleteMe(this.event).subscribe((resp) => {
