...
import { BusinessMenuServiceService } from "../../services/rpx-logged-in/business-menu/business-menu-service.service";
...
export class RewardMenuShortComponent implements OnInit {
  ...
  user$ = new BehaviorSubject<{ user: User; rpx_user: RpxUser }>(null);
  loyaltyPointsBalance$ = new BehaviorSubject<LoyaltyPointBalance>(null);
  _user: { user: User; rpx_user: RpxUser };
  ...
  rewardUser(reward: Reward) {
    ...
    const c = confirm(
      `Are you sure you want to reward this user with "${reward.name}"?`,
    );
    if (!c) {
    ...
    this.loyaltyPointService
      .redeem(reward, this._user.rpx_user.id)
      .pipe(
  ...
  userHasEnoughLp(reward: Reward): boolean {
    ...
    let point_cost =
      (reward.point_cost / balance.loyalty_point_dollar_percent_value) * 100;
      ...
      .pipe(
        ...
        tap((rewardList) => {
          const availableRewards = rewardList?.filter(
            (r) => this.userIsInTier(r) && this.userHasEnoughLp(r),
          );
  ...
  rewardTileStyling(reward: Reward) {
    if (reward.type === 0) return { background: "url(" + reward.images + ")" };
    else return { background: "#64e56f" };
  }
}
