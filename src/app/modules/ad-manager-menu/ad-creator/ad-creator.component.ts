...
import { AdCreatorService } from "../../../services/rpx-logged-in/ad-manager-menu/ad-creator/ad-creator.service";
...
const AD_MEDIA_UPLOAD_API_URL = `${rpxGlobals.API}in-house/upload-media`;
...
export class AdCreatorComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.adTypeList.forEach((inHouse) => {
      if (
        inHouse.type === "footer" &&
        this.user.userSubscriptionPlan ===
  ...
  saveAd() {
    ...
    if (!this.ad) {
      this.adCreatorService
        .saveAd(adObj)
        .subscribe((resp) => this.saveAdCb(resp));
    } else {
      adObj.id = this.ad.id;
      this.adCreatorService
        .updateAd(adObj)
  ...
  deleteMe() {
    ...
    if (r) {
      this.adCreatorService
        .deleteMe(this.ad)
        .subscribe((resp) => this.deleteMeCb(resp));
    }
  }
