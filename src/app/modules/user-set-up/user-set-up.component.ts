...
export class UserSetUpComponent implements OnInit {
  ...
  get customerEmail() {
    return this.accountSetUpForm.get("customerEmail").value;
  }
  get customerFirstName() {
    return this.accountSetUpForm.get("customerFirstName").value;
  }
  get h() {
  ...
  get customerPhoneNumber() {
    return this.accountLookUpForm.get("customerPhoneNumber").value;
  }
  get g() {
    return this.accountLookUpForm.controls;
  }
  ...
  lookEmUp() {
    ...
    if (this.accountLookUpForm.invalid) {
      ...
      this.phoneNumberLabel.nativeElement.innerHTML =
  ...
  lookEmUpServiceCall() {
    this.loyaltyPointsService
      .lookUpCustomer(this.customerPhoneNumber)
      .pipe(
        debounceTime(2000),
        catchError((err) => {
        ...
        tap(
          (res: {
            ...
            if (res && res.user && res.rpx_user) {
              this.user$.next({
                user: res.user,
                rpx_user: res.rpx_user,
  ...
  setUpLpTapButton() {
    this.tapEventSub$ = this.tapEvent$
      .pipe(
  ...
  removeWhiteSpace(key) {
    this.accountSetUpForm
      .get(key)
      .setValue(this.accountSetUpForm.get(key).value.trim());
  }
}
